...
class OpenAIService
{
     ...
     * @return array{recommended_price: float|null, message: string|null}
     */
    public function recommendMarketPrice(array $context): array
    {
        $response = Http::withToken(config('services.openai.key'))
            ->acceptJson()
            ->connectTimeout(5)
            ...
            ->post('https://api.openai.com/v1/responses', [
                ...
                'input' => [
                    [
                        'role' => 'system',
                        'content' => <<<'PROMPT'
You recommend fair Philippine agricultural surplus and waste-resource prices.
Use only the supplied farmer location and verified market-price summary.
The recommendation must stay inside the supplied minimum and maximum price.
Return only valid JSON with this exact structure:
{"recommended_price": 0, "message": ""}
recommended_price must be a number per kilogram.
message must briefly explain the location and market-price basis in at most 25 words.
Do not use markdown or add information outside the JSON.
PROMPT,
                    ],
                    [
                        'role' => 'user',
        ...
        return [
            'recommended_price' => isset($result['recommended_price'])
                ? (float) $result['recommended_price']
                : null,
